...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare query to check manager credentials
    $stmt = $conn->prepare("SELECT id, hotel_id, username, password FROM managers WHERE username=?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $stmt->store_result();

    if($stmt->num_rows == 1){
        $stmt->bind_result($id, $hotel_id, $db_username, $db_password);
        $stmt->fetch();
...
<head>
<meta charset="UTF-8">
<title>Manager Login | DineFlow</title>
